    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="text-xl font-semibold leading-tight text-gray-800">
                Catálogo — Tipos de Impuesto
            </h2>
            <a href="{{ route('catalogos.impuestos.create') }}"
                    ...
                    <tbody class="divide-y divide-gray-100">
                        @forelse($impuestos as $impuesto)
                            <tr class="transition hover:bg-gray-50">
                                <td class="px-5 py-3 font-medium text-gray-900">{{ $impuesto->nombre }}</td>
                                <td class="px-5 py-3">
                                    <span class="inline-block px-2 py-0.5 rounded bg-amber-50 text-amber-700 font-mono font-semibold text-xs">
                                        {{ $impuesto->clave }}
                                    </span>
                                </td>
                                ...
                                <td class="px-5 py-3 text-right">
                                    <div class="inline-flex items-center gap-2">
                                        <a href="{{ route('catalogos.impuestos.edit', $impuesto) }}"
                                           class="font-medium text-indigo-600 hover:text-indigo-800">Editar</a>

                                        <form action="{{ route('catalogos.impuestos.toggle', $impuesto) }}"
                                              method="POST" class="inline">
                                            @csrf @method('PATCH')
                                            <button type="submit"
                                                    class="{{ $impuesto->activo ? 'text-amber-600 hover:text-amber-800' : 'text-green-600 hover:text-green-800' }} font-medium">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-5 py-10 text-center text-gray-400">
                                    No hay tipos de impuesto registrados.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
